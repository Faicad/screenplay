...
const LAYOUT = {
  h: {
    text1: { top: 14, align: 'center', pad:1, fontSize: 160 },
    text2: { top: 80, align: 'center', pad:1, fontSize: 160  },
  },
  v: {
    text1: { top: 22, align: 'center', fontSize: 200 },
    text2: { top: 75, align: 'center', fontSize: 200  },
  },
}
...
body{width:${w}px;height:${h}px;${bgStyle};position:relative;overflow:hidden}
${hasCoverOverlay ? '.bg-blur{position:absolute;top:-10%;left:-10%;width:120%;height:120%;object-fit:cover;filter:blur(50px);z-index:0;opacity:.95}' : ''}
.t{font-weight:bold;font-family:'Microsoft YaHei','PingFang SC','Noto Sans CJK SC','Noto Sans CJK',sans-serif;filter:drop-shadow(0 2px 4px rgba(0,0,0,.85)) drop-shadow(0 6px 28px rgba(0,0,0,.45));line-height:1.2;position:absolute;word-break:keep-all;z-index:2}.overlay{position:absolute;top:0;left:0;width:100%;height:100%;object-fit:contain;z-index:1}
.text1{${text1Css}}
.text2{${text2Css}}
</style></head><body>
${hasCoverOverlay ? `<img class="bg-blur" src="${coverImgUrl}">` : ''}
