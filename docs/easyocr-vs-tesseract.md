...
测试命令：
```bash
python mark-text.py screenshot/WorkBuddy_v.png "专家:left" "技能:top13-center50" "3d模型查看:top15" "SkillHub:top"
python mark-text.py screenshot/WorkBuddy_h.png "专家:left" "技能:top13-center50" "3d模型查看:top15" "SkillHub:top"
```
...
- `mark-text.py` — Tesseract 版本（5 层回退策略，见 `docs/ocr-fallback-strategy.md`）
- `mark-text-easyocr.py` — EasyOCR 版本（3 轮预处理：原图 + CLAHE + 反色）

## 使用方式

```bash
# EasyOCR (推荐，中英文混合场景更准)
python mark-text-easyocr.py <截图.png> "文字:区域" ...

# Tesseract (更快，纯英文或大文本场景可用)
python mark-text.py <截图.png> "文字:区域" ...
```
