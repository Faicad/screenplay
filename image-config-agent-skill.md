...
例如用户提供了 `screenshot/win.png`，没有 `_h.png` / `_v.png` 版本：
...
`easyocr-mark.mjs` 一次性完成 easyocr 分析 + 坐标写入：

```bash
node easyocr-mark.mjs <截图.png> <输出marks.json> <查找文字1> [查找文字2 ...]
```
...
node easyocr-mark.mjs \
  e1/ai_gen/m0_refactor_0000_h_full.png \
  e1/ai_gen/m0_refactor_0000_h_marks.json \
  "3D查看器"

# 竖屏
node easyocr-mark.mjs \
  e1/ai_gen/m0_refactor_0000_v_full.png \
  e1/ai_gen/m0_refactor_0000_v_marks.json \
  "3D查看器"
```

注意：截图在 `aiGenDir/` 中的文件名格式为 `<scriptName>_<NNNN>_<h|v>_full.png`，由 `generate-image2-video.mjs` 自动从 `screenshot/` 复制过去。如果尚未生成，可先对原图执行一次视频生成（`--no-tts --no-burn`），或手动复制。
...
**"分成N段显示" 用一条 caption 加完整文字** — 当 description 要求一段文字"分成N段显示"时，不要拆成多个 caption 条目。用一条 caption，text 放完整文字，html-composer 内部会处理分段动画效果。参见 `e1/m5.mjs` 的 caption `"求关注、求转发、求收藏"` 写法（第75-88行）。
...
```bash
node generate-image2-video.mjs <script.mjs> [--tts edge-tts] [--no-tts] [--no-burn] [-f]
```
...
```bash
# Step 1: TTS
node pregen-tts.mjs e1/m0_refactor.mjs

# Step 2: EasyOCR 定位文字并写入 marks.json
# (截图由 generate-image2-video.mjs 自动复制到 aiGenDir/，可先跑一遍视频生成)
node easyocr-mark.mjs e1/ai_gen/m0_refactor_0000_h_full.png e1/ai_gen/m0_refactor_0000_h_marks.json "3D查看器"
node easyocr-mark.mjs e1/ai_gen/m0_refactor_0000_v_full.png e1/ai_gen/m0_refactor_0000_v_marks.json "3D查看器"
node easyocr-mark.mjs e1/ai_gen/m0_refactor_0002_h_full.png e1/ai_gen/m0_refactor_0002_h_marks.json "详细信息"
node easyocr-mark.mjs e1/ai_gen/m0_refactor_0002_v_full.png e1/ai_gen/m0_refactor_0002_v_marks.json "详细信息"

# Step 3: 补全 m0_refactor.mjs + 写 marks.json

# Step 4: 生成视频
node generate-image2-video.mjs e1/m0_refactor.mjs
```

## 参考脚本

参见 `e1/m0_refactor.mjs` — `image_config` 格式的完整参考脚本。
