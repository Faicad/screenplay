...
try {
  for (const orient of orientations) {
    ...
    const baseFs = fontSizeForWidth(w, '3D建模、预览、打印')
    const fs = orient === 'h' ? Math.round(baseFs * 0.5) : baseFs
    const finalPath = join(genDir, `${projectName}_cover_final_${orient}.png`)

    const isH = orient === 'h'
    const aiColor = 'color:#F0D898;background-image:linear-gradient(180deg,#F8ECD0,#F0D898,#E4C878);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent'
    const aiCss = isH
      ? 'top:25%;left:50%;text-align:center;transform:translate(-50%,-50%);' + aiColor
      : 'top:30%;left:50%;text-align:center;transform:translate(-50%,-50%);' + aiColor
    const textColor = 'color:#C4D8EC;background-image:linear-gradient(180deg,#E0ECF8,#C4D8EC,#A8C4E0);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent'
    const textCss = isH
      ? 'top:75%;left:50%;;text-align:center;transform:translate(-50%,-50%);' + textColor
      : 'top:75%;left:50%;text-align:center;transform:translate(-50%,-50%);' + textColor
...
<div class="t ai" id="aiEl">3D建模、预览、打印</div>
<div class="t text" id="textEl">一个Skill搞定</div>
<script>
